<?php
/**
 * Renders a compact event card for a given product (event) ID.
 * Keeps markup consistent across lists.
 *
 * @param int $post_id Product ID.
 */
function tapin_render_compact_event_card($post_id) {
    if (!$post_id || get_post_type($post_id) !== 'product') return;

    $thumb_url = get_the_post_thumbnail_url($post_id, 'medium');
    if (!$thumb_url && function_exists('wc_placeholder_img_src')) $thumb_url = wc_placeholder_img_src();
    if (!$thumb_url) $thumb_url = includes_url('images/media/default.png');

    // Use unified meta key if defined, fallback to 'event_date'
    $meta_key = defined('TAPIN_META_EVENT_DATE') ? TAPIN_META_EVENT_DATE : 'event_date';
    $event_date_raw = get_post_meta($post_id, $meta_key, true);
    $event_date_formatted = $event_date_raw ? date_i18n('j M, Y', strtotime($event_date_raw)) : '';
    ?>
    <a href="<?php echo esc_url(get_permalink($post_id)); ?>" class="tapin-event-card" title="<?php echo esc_attr(get_the_title($post_id)); ?>">
        <img src="<?php echo esc_url($thumb_url); ?>" alt="<?php echo esc_attr(get_the_title($post_id)); ?>" class="tapin-event-card__image">
        <div class="tapin-event-card__content">
            ...
            <p class="tapin-event-card__date">
                <svg class="tapin-event-card__date-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
...
/**
 * Shortcode: [producer_events_grid]
 * Renders "active" and "past" event grids on a producer's profile.
 * Will return empty output for profiles that are not Producer/Owner.
 */
add_shortcode('producer_events_grid', 'tapin_render_producer_events_grid');

function tapin_render_producer_events_grid($atts) {
    $producer_id = 0;

    // UM profile owner (preferred)
    if (function_exists('um_profile_id')) {
        $producer_id = (int) um_profile_id();
    }

    // Fallback to current user only if they are Producer/Owner
    ...
    // Gate: show only for Producer/Owner profiles (admins are optional; keep strict)
    $profile_user = get_userdata($producer_id);
    if (!$profile_user) return '';
    $is_producer_profile = (bool) array_intersect((array) $profile_user->roles, ['producer', 'owner']);
    if (!$is_producer_profile) return ''; // non-producer profile => no grids

    $meta_key = defined('TAPIN_META_EVENT_DATE') ? TAPIN_META_EVENT_DATE : 'event_date';
    $current_datetime = current_time('mysql');

    // Active / upcoming events
    $active_events_query = new WP_Query([
        'post_type'      => 'product',
        'post_status'    => 'publish',
        'author'         => $producer_id,
        'posts_per_page' => -1,
    ...
    <style>
        ...
        .tapin-box .tapin-section-title {
            font-size: 1rem;
            font-weight: 700;
            color: var(--tapin-primary-color);
            margin: 0 0 10px 0;
            padding-bottom: 6px;
            border-bottom: 1px solid var(--tapin-border-color);
        }
        .tapin-events-grid {
            display: grid;
    ...
    <div class="tapin-events-wrapper">
        <div class="tapin-box">
            ...
            <?php if ($active_events_query->have_posts()) : ?>
                <div class="tapin-events-grid">
                    <?php while ($active_events_query->have_posts()) : $active_events_query->the_post();
                        tapin_render_compact_event_card(get_the_ID());
                    endwhile; ?>
                </div>
            <?php else : ?>
        ...
        <div class="tapin-box">
            ...
            <?php else : ?>
                <p class="tapin-no-events">אין אירועים בארכיון.</p>
            <?php endif; wp_reset_postdata(); ?>
        </div>
    </div>
    <?php
