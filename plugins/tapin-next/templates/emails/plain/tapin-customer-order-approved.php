...
echo esc_html__('המפיק אישר את כל המשתתפים בהזמנה שלך.', 'tapin') . "\n\n";

if ($event_name_plain !== '' || $event_date_plain !== '' || $event_location_plain !== '') {
    if ($event_name_plain !== '') {
        echo sprintf( esc_html__('שם האירוע: %s', 'tapin'), $event_name_plain ) . "\n";
    }
    if ($event_date_plain !== '') {
        echo sprintf( esc_html__('תאריך ושעה: %s', 'tapin'), $event_date_plain ) . "\n";
    }
    if ($event_location_plain !== '') {
        echo sprintf( esc_html__('מיקום: %s', 'tapin'), $event_location_plain ) . "\n";
    }
    echo "\n";
}

if ($order_number_plain !== '') {
    echo sprintf( esc_html__('מספר ההזמנה: %s', 'tapin'), $order_number_plain ) . "\n";
}
if ($order_total !== '') {
    echo sprintf( esc_html__('סכום החיוב הסופי: %s', 'tapin'), $order_total ) . "\n";
}
echo "\n";
