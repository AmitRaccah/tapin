...
final class ProducerEventSales implements Service {
    ...
    public function render($atts): string {
        ...
        $vendor_id = 0;
        if ($a['vendor']==='current') $vendor_id = get_current_user_id();
        ...
        if (!$can_view_all && get_current_user_id() !== $vendor_id) { status_header(403); return '<p style="direction:rtl;text-align:right">אין לך הרשאה לצפות בדוח של משתמש אחר.</p>'; }
        ...
        $args1 = ['limit'=>-1,'type'=>'shop_order','status'=>$statuses?:['wc-processing','wc-completed'],'meta_key'=>'_dokan_vendor_id','meta_value'=>$vendor_id,'return'=>'ids'];
        if ($date_after || $date_before) $args1['date_created'] = array_filter(['after'=>$date_after?:null,'before'=>$date_before?:null,'inclusive'=>true]);
        $order_ids = wc_get_orders($args1);

        $rows=[];
        $accumulate=function($oids) use (&$rows,$get_thumb){
            foreach ($oids as $oid){
                $order=wc_get_order($oid); if(!$order) continue;
                foreach ($order->get_items('line_item') as $item){
                    $pid=(int)$item->get_product_id(); if(!$pid) continue;
                    if (!isset($rows[$pid])) $rows[$pid]=['name'=>get_the_title($pid),'qty'=>0,'sum'=>0.0,'view'=>get_permalink($pid),'thumb'=>$get_thumb($pid)];
        ...
        if (!empty($order_ids)) {
            ...
            $args2=['limit'=>-1,'type'=>'shop_order','status'=>$statuses?:['wc-processing','wc-completed'],'return'=>'ids'];
            if ($date_after || $date_before) $args2['date_created']=array_filter(['after'=>$date_after?:null,'before'=>$date_before?:null,'inclusive'=>true]);
        ...
        <div class="tapin-sales-simple" dir="rtl" style="text-align:right">
          ...
          <table class="widefat striped">
            <thead><tr><th style="width:68px">תמונה</th><th>אירוע</th><th>נמכרו</th><th>סכום</th></tr></thead>
            <tbody>
